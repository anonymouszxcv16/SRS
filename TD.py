...
class Hyperparameters:
    ...
    # IQL / AW-BC
    beta_iql: float = 3.0  # temperature for advantage weighting
...
class Agent(object):
    ...
    def train(self):
        ...
        state, action, next_state, reward, mc_score, not_done = self.replay_buffer.sample()
        ...
        with torch.no_grad():
            ...
            if self.args.normalize == 1:
                reward = (reward / self.replay_buffer.reward.max().item()).clamp(self.replay_buffer.reward.min().item(), 1)
            ...
            if "SRS" in self.args.policy or "RC" in self.args.policy:
                r_avg, r_max = self.replay_buffer.reward[:self.replay_buffer.size].mean(), self.replay_buffer.reward[:self.replay_buffer.size].max()
                reward = reward.expand((self.replay_buffer.batch_size, self.args.N)).clone()
                r_rc = (reward - r_avg) / r_max

                if "SRS" in self.args.policy:
                    reward = (1 / self.args.alpha) * (1 + (self.args.alpha * (r_rc)).exp()).log()

                    if self.args.auto_alpha == 1:
                        rewards_std = self.replay_buffer.reward.std()

                        self.std_sum += rewards_std

                        if (self.training_steps + self.args.timesteps_before_training) % self.args.auto_alpha_interval == 0:
                            std_mean = self.std_sum / self.args.auto_alpha_interval
                            self.args.alpha = self.scaled_sigmoid(std_mean)
        ...
        if self.training_steps % self.hp.policy_freq == 0:
            ...
            self.bc_loss += F.mse_loss(actor, action)
            ...
            if "REINFORCE" in self.args.policy:
                # log policy * Gt
                gradient = (log_prob_a_tilda * mc_score).sum()
